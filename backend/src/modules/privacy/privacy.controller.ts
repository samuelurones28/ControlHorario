...
export const checkPrivacyConsent = async (req: FastifyRequest, reply: FastifyReply) => {
...
export const acceptPrivacyConsent = async (req: FastifyRequest<{ Body: { version: string, ipAddress?: string } }>, reply: FastifyReply) => {
  const user = req.user as any;
  const employeeId = user.employeeId;
...
export const getPrivacyPolicy = async (req: FastifyRequest, reply: FastifyReply) => {
  const policy = `
