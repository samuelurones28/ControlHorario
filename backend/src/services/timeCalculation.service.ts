...
import { startOfDay, endOfDay, endOfMonth, differenceInMilliseconds } from 'date-fns';

export class TimeCalculationService {
  
  private async getEffectiveEntries(employeeId: string, from: Date, to: Date) {
  ...
  async calculateDailyRecord(employeeId: string, date: Date, employeeHours?: number) {
    if (!employeeHours) {
    ...
    for (const entry of entries) {
      if (entry.entryType === 'CLOCK_IN') {
        ...
        if (lastClockIn) {
          totalWorked += differenceInMilliseconds(entry.effectiveTimestamp, lastClockIn);
          lastClockIn = null;
        }
        ...
        if (lastPauseStart) {
          totalPaused += differenceInMilliseconds(entry.effectiveTimestamp, lastPauseStart);
          lastPauseStart = null;
        }
        lastClockIn = entry.effectiveTimestamp;
      }
  ...
  async calculateWeeklyHours(employeeId: string, weekStart: Date) {
    const emp = await prisma.employee.findUnique({ where: { id: employeeId } });
  ...
  async calculateMonthlyReport(employeeId: string, month: number, year: number) {
     const emp = await prisma.employee.findUnique({ where: { id: employeeId } });
